...
const statusColor: Record<string, string> = {
  ...
  NO_SHOW: 'bg-muted border-border',
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-lg font-bold text-text-primary">تقویم نوبت‌ها</h2>
        <div className="flex items-center gap-2">
          <div className="flex glass rounded-xl p-1">
            {(['daily', 'weekly', 'list'] as const).map(v => (
              <button key={v} onClick={() => setView(v)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${view === v ? 'gradient-primary text-white' : 'text-text-secondary hover:text-text-primary'}`}>
      ...
      <div className="flex items-center justify-between">
        <button onClick={view === 'weekly' ? prevWeek : prevDay} className="p-2 rounded-xl glass hover:bg-muted"><ChevronRight className="w-5 h-5 text-text-secondary" /></button>
        <div className="text-center">
          <div className="text-sm font-bold text-text-primary">{toPersianDigits(jDate.jd)} {jDate.monthName} {toPersianDigits(jDate.jy)}</div>
          <div className="text-xs text-text-secondary">{DAYS[(currentDate.getDay() + 1) % 7]}</div>
        </div>
        <button onClick={view === 'weekly' ? nextWeek : nextDay} className="p-2 rounded-xl glass hover:bg-muted"><ChevronLeft className="w-5 h-5 text-text-secondary" /></button>
      </div>

      {view === 'daily' && (
        <GlassCard className="p-4">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs text-text-secondary">نمای روزانه</span>
          ...
          {smartSlots.length > 0 && (
            <div className="mb-3 p-3 rounded-xl bg-cyan-500/10 border border-cyan-500/20">
              <div className="text-xs font-medium text-primary-light mb-2">زمان‌های آزاد پیشنهادی:</div>
              <div className="flex flex-wrap gap-2">
                {smartSlots.map((slot, i) => (
                  <div key={i} className="px-3 py-1 rounded-lg bg-cyan-500/10 text-xs text-primary-light border border-cyan-500/20">
          ...
          {loading ? (
            ...
            <div className="space-y-1">
              {timeSlots.slice(8, 20).map((slot, i) => {
                ...
                return (
                  <div key={slot} className="flex items-start gap-3 min-h-[48px]">
                    <div className="w-12 text-xs text-text-secondary pt-1.5">{toPersianDigits(slot)}</div>
                    <div className="flex-1 space-y-1">
                      {appts.map(a => (
                        <button key={a.id} onClick={() => setSelectedAppt(a)} className={`w-full text-right p-2 rounded-lg border ${statusColor[a.status] || statusColor.PENDING} hover:scale-[1.01] transition-all`}>
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-medium text-text-primary">{a.customer.name}</span>
                            <GlassBadge variant={statusVariant[a.status]}>{statusLabel[a.status]}</GlassBadge>
                          </div>
                          <div className="text-xs text-text-secondary mt-0.5">{a.service.name} - {a.staff.name}</div>
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              {appointments.length === 0 && <p className="text-sm text-text-secondary text-center py-8">نوبتی برای این روز ثبت نشده است</p>}
            </div>
          )}
        </GlassCard>
      )}
                ...
                <div className="space-y-1">
                  {dayAppts.map(a => (
                    <button key={a.id} onClick={() => setSelectedAppt(a)} className={`w-full text-right p-1.5 rounded-lg border ${statusColor[a.status]} text-xs`}>
                      <div className="font-medium text-text-primary truncate">{a.customer.name}</div>
                      <div className="text-text-secondary truncate">{new Date(a.startTime).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' })}</div>
                    </button>
                  ))}
                </div>
              </GlassCard>
      ...
      {view === 'list' && (
        <GlassCard className="overflow-hidden">
          {appointments.length === 0 ? (
            <p className="text-sm text-text-secondary text-center py-8">نوبتی برای این روز ثبت نشده است</p>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {appointments.map(a => (
                  <tr key={a.id} onClick={() => setSelectedAppt(a)} className="border-b border-border hover:bg-surface/3 cursor-pointer">
                    <td className="p-3 text-text-primary">{new Date(a.startTime).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' })}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <GlassCard strong className="relative p-6 w-full max-w-md animate-scale-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSelectedAppt(null)} className="text-text-secondary"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-text-secondary">مشتری:</span><span className="text-text-primary">{selectedAppt.customer.name}</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">موبایل:</span><span className="text-text-primary">{toPersianDigits(selectedAppt.customer.mobile)}</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">خدمت:</span><span className="text-text-primary">{selectedAppt.service.name}</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">کارکن:</span><span className="text-text-primary">{selectedAppt.staff.name}</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">زمان:</span><span className="text-text-primary">{new Date(selectedAppt.startTime).toLocaleString('fa-IR')}</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">وضعیت:</span><GlassBadge variant={statusVariant[selectedAppt.status]}>{statusLabel[selectedAppt.status]}</GlassBadge></div>
              {selectedAppt.internalNote && <div className="p-3 rounded-xl bg-surface/5"><span className="text-text-secondary text-xs">یادداشت:</span><p className="text-text-primary mt-1">{selectedAppt.internalNote}</p></div>}
            </div>
            <div className="mt-4 space-y-2">
              <div className="flex flex-wrap gap-2">
                {['CONFIRMED', 'COMPLETED', 'NO_SHOW'].map(st => (
                  <button key={st} onClick={() => changeStatus(selectedAppt.id, st)} className="px-3 py-1.5 rounded-lg glass text-xs text-text-primary hover:bg-muted">{statusLabel[st]}</button>
                ))}
