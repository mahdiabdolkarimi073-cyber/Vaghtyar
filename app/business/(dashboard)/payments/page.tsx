...
export default function PaymentsPage() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-bold text-primary-custom">تاریخچه پرداخت‌ها</h2>
      ...
      <GlassCard className="p-4">
        {loading ? (
          ...
          <div className="text-center py-12">
            <Receipt className="w-12 h-12 text-secondary-custom mx-auto mb-3 opacity-50" />
            <p className="text-sm text-secondary-custom">پرداختی ثبت نشده است</p>
          </div>
        ) : (
          <>
                ...
                <tbody>
                  {payments.map(p => (
                    <tr key={p.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                      <td className="p-3 text-primary-custom font-medium">{p.planName}</td>
                      <td className="p-3 text-secondary-custom">{formatPrice(p.amount)}</td>
                      ...
                      <td className="p-3 text-secondary-custom text-xs">{p.refId ? toPersianDigits(p.refId) : '-'}</td>
                      <td className="p-3 text-secondary-custom text-xs">{formatDateShortFA(new Date(p.createdAt))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
