    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => <div key={i} className="glass h-28 rounded-2xl glass-shimmer" />)}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => <div key={i} className="glass h-28 rounded-2xl glass-shimmer" />)}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <GlassCard className="p-5">
          <h3 className="text-sm font-bold text-primary-custom mb-4">درآمد ۷ روز اخیر</h3>
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={charts?.revenue || []}>
              ...
              <XAxis dataKey="jalaliDate" tick={{ fill: 'rgba(248,250,252,0.4)', fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: 'rgba(248,250,252,0.4)', fontSize: 11 }} axisLine={false} tickLine={false} />
        ...
        <GlassCard className="p-5">
          <h3 className="text-sm font-bold text-primary-custom mb-4">نوبت‌های ۷ روز اخیر</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={charts?.appointments || []}>
              ...
              <YAxis tick={{ fill: 'rgba(248,250,252,0.4)', fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: 'rgba(18,18,30,0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: '#f8fafc' }} />
              <Bar dataKey="confirmed" stackId="a" fill="#6366f1" radius={[0, 0, 0, 0]} />
      ...
      <GlassCard className="p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-primary-custom">نوبت‌های امروز</h3>
          <GlassBadge variant="primary">{toPersianDigits(upcoming.length)} نوبت</GlassBadge>
        </div>
        {upcoming.length === 0 ? (
          <p className="text-sm text-secondary-custom text-center py-8">نوبتی برای امروز باقی نمانده است</p>
        ) : (
          <div className="space-y-2">
            {upcoming.map((app) => {
              const time = new Date(app.startTime).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' });
              return (
                <div key={app.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 hover:bg-white/8 transition-all">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl gradient-primary flex items-center justify-center text-white text-xs font-bold">
                      {time}
                    </div>
                    <div>
                      <div className="text-sm font-medium text-primary-custom">{app.customerName}</div>
                      <div className="text-xs text-secondary-custom">{app.serviceName} - {app.staffName}</div>
                    </div>
                  </div>
      ...
      {(stats?.pendingAppointments ?? 0) > 0 && (
        <GlassCard className="p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-primary-custom">یادآوری‌ها</h3>
          </div>
          <div className="flex items-center justify-between p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
            <span className="text-sm text-amber-300">{toPersianDigits(stats?.pendingAppointments ?? 0)} نوبت در انتظار تایید</span>
