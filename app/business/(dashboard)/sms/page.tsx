...
export default function SmsPage() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-bold text-text-primary">پنل پیامک</h2>
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <GlassCard className="p-4">
            ...
            {quotaUnlimited ? (
              ...
              <>
                ...
                <div className="h-1.5 rounded-full bg-surface/5 overflow-hidden mt-2">
          ...
          <GlassCard className="p-4">
            <div className="text-xs text-text-secondary mb-1">ارسال شده</div>
            <div className="text-xl font-bold text-secondary">{toPersianDigits(stats.totalSent)}</div>
          </GlassCard>
          <GlassCard className="p-4">
            <div className="text-xs text-text-secondary mb-1">شبیه‌سازی شده</div>
            <div className="text-xl font-bold text-warning">{toPersianDigits(stats.totalSimulated)}</div>
          </GlassCard>
          <GlassCard className="p-4">
            <div className="text-xs text-text-secondary mb-1">ناموفق</div>
            <div className="text-xl font-bold text-error">{toPersianDigits(stats.totalFailed)}</div>
          </GlassCard>
        </div>
      )}

      {quotaLimit !== null && quotaLimit !== undefined && !quotaUnlimited && quotaPercent >= 100 && (
        <div className="p-4 rounded-xl bg-error/10 border border-red-500/20 flex items-center justify-between">
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {logs.map(log => (
                    <tr key={log.id} className="border-b border-border hover:bg-surface/3 cursor-pointer transition-colors" onClick={() => setSelectedLog(log)}>
                      <td className="p-3 text-text-primary text-xs">{smsTypeLabel[log.smsType] || log.smsType}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <GlassCard strong className="relative p-6 w-full max-w-md animate-scale-in">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSelectedLog(null)} className="text-text-secondary"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-text-secondary">نوع:</span><span className="text-text-primary">{smsTypeLabel[selectedLog.smsType] || selectedLog.smsType}</span></div>
              ...
              <div className="flex justify-between"><span className="text-text-secondary">شماره:</span><span className="text-text-primary">{toPersianDigits(selectedLog.recipientPhone)}</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">وضعیت:</span><GlassBadge variant={statusVariant[selectedLog.status] || 'default'}>{statusLabel[selectedLog.status] || selectedLog.status}</GlassBadge></div>
              <div><span className="text-text-secondary block mb-1">متن پیام:</span><p className="text-text-primary p-3 rounded-xl bg-surface/5">{selectedLog.messageBody}</p></div>
              <div className="text-xs text-text-secondary">{formatDateShortFA(new Date(selectedLog.createdAt))}</div>
            </div>
          </GlassCard>
        </div>
      )}
