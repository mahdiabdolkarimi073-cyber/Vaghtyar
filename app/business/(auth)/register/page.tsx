      ...
      <>
        ...
        <div className="w-full max-w-md mx-auto">
          <GlassCard className="p-8 text-center animate-scale-in">
            <div className="inline-flex w-20 h-20 rounded-full bg-amber-500/15 items-center justify-center mb-6">
              <Loader2 className="w-10 h-10 text-amber-400 animate-spin" />
            </div>
            <h2 className="text-xl font-bold text-primary-custom mb-3">در انتظار تایید مدیر</h2>
      ...
      <div className="w-full max-w-lg mx-auto">
        <GlassCard className="p-8 animate-scale-in">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-secondary-custom mt-2">پنل مدیریت نوبت‌یار برای کسب‌وکار شما</p>
          </div>
            ...
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-all duration-300 ${
                  step >= s ? 'gradient-primary text-white' : 'bg-white/5 text-secondary-custom border border-white/10'
                }`}>
                  {step > s ? <CheckCircle2 className="w-4 h-4" /> : s}
                </div>
                {s < 3 && <div className={`w-12 h-0.5 rounded-full transition-all duration-300 ${step > s ? 'bg-indigo-500' : 'bg-white/10'}`} />}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fade-in">
              <p className="text-sm font-medium text-secondary-custom mb-4">اطواع صاحب کسب‌وکار</p>
          ...
          {step === 2 && (
            <div className="space-y-4 animate-fade-in">
              <p className="text-sm font-medium text-secondary-custom mb-4">اطلاعات کسب‌وکار</p>
              <GlassInput {...register('name')} placeholder="نام کسب‌وکار" error={errors.name?.message} />
              <div>
                <label className="block text-sm font-medium text-secondary-custom mb-1.5">دسته‌بندی</label>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep(1)} className="flex-1 py-2.5 rounded-xl border border-white/10 text-secondary-custom hover:bg-white/5 transition-all text-sm">
                  مرحله قبل
                </button>
          ...
          {step === 3 && (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 animate-fade-in">
              <p className="text-sm font-medium text-secondary-custom mb-4">بررسی و تایید</p>
              <GlassCard className="p-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-secondary-custom">نام:</span><span>{watch('ownerFirstName')} {watch('ownerLastName')}</span></div>
                <div className="flex justify-between"><span className="text-secondary-custom">موبایل:</span><span>{watch('ownerMobile')}</span></div>
                <div className="flex justify-between"><span className="text-secondary-custom">ایمیل:</span><span>{watch('email')}</span></div>
                <div className="flex justify-between"><span className="text-secondary-custom">کسب‌وکار:</span><span>{watch('name')}</span></div>
                <div className="flex justify-between"><span className="text-secondary-custom">دسته‌بندی:</span><span>{watch('category')}</span></div>
                <div className="flex justify-between"><span className="text-secondary-custom">محله:</span><span>{watch('neighborhood')}</span></div>
                <div className="flex justify-between"><span className="text-secondary-custom">تلفن:</span><span>{watch('phone')}</span></div>
              </GlassCard>
              <div className="flex gap-3">
                <button type="button" onClick={() => setStep(2)} className="flex-1 py-2.5 rounded-xl border border-white/10 text-secondary-custom hover:bg-white/5 transition-all text-sm">
                  مرحله قبل
                </button>
          ...
          <div className="mt-6 text-center text-sm text-secondary-custom">
            حساب دارید؟{' '}
            <Link href="/business/login" className="text-indigo-400 hover:text-indigo-300 font-medium">
              وارد شوید
            </Link>
          </div>
        </GlassCard>
